...
class ThumbnailGenerator:
    ...
    def generate_image(vertices, width=300, height=300):
        ...
        # Save to buffer
        buf = io.BytesIO()
        plt.savefig(buf, format='jpg', dpi=100, facecolor='#252525') # Use jpg to match user request
        plt.close(fig)
        buf.seek(0)
        return buf.read()
    ...
    def inject_thumbnail(gcode_path: Path, stl_path: Path):
        ...
        print(f"Injecting thumbnail into {gcode_path} from {stl_path}")
        try:
            vertices = ThumbnailGenerator.load_stl(stl_path)
            
            # Generate 32x32 (mini) and 400x400 (large) - matching standard Cura/Mainsail sizes
            # User example: 50x50 and 217x217. 
            # We will use 32x32 (standard tiny) and 300x300 (decent quality) or 400x400
            images = [
